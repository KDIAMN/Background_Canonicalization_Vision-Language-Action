...
def init_module(
    ...
    Args:
        ...
        to_bf16 (bool): Whether to convert to torch.float16 data type.
...
def run_forward_pass(
    ...
    ground_truth_actions = batch["actions"].to(device_id).to(torch.float16)
    ...
    # Compute metrics for continuous action representations (L1 regression)
        ...
        for item in output.hidden_states[0:]:
            ...
            # actions_hidden_states = text_hidden_states[:, -1, :].reshape(batch_size, 1, -1).to(torch.float16)
            actions_hidden_states = text_hidden_states[current_action_mask | next_actions_mask].reshape(batch_size, 1,NUM_TOKENS, -1).to(torch.float16)
...
def save_training_checkpoint(
    ...
    if cfg.use_lora and cfg.merge_lora_during_training:
        if cfg.use_minivlm:
            ...
            base_vla = AutoModelForVision2Seq.from_config(config, torch_dtype=torch.float16)  # Create a new model with configuration, the parameters are randomly initialized
            ...
            cfg.config_file_path, torch_dtype=torch.float16, low_cpu_mem_usage=False, trust_remote_code=False
        )
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if cfg.use_minivlm:
        hf_token = ''
        if 'prism-qwen25-extra-dinosiglip-224px-0_5b' in cfg.vlm_path or os.path.isdir(cfg.vlm_path):
        ...
        config = AutoConfig.from_pretrained("pretrained_models/configs/config.json")
        vla = AutoModelForVision2Seq.from_config(config, torch_dtype=torch.float16).to(device_id)  # Create a new model with configuration, the parameters are randomly initialized
