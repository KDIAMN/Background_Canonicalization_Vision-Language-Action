#!/bin/bash
# LIBERO-PRO Environment(libero_object_temp) 평가 — normal_libero_object 모델
# - 학습/추론 일치 설정:
...
#
# 결과 저장 위치 (기본):
#   /path/to/VLA_Adapter/eval/libero_pro_env_object/Normal_libero_pro_object/
#       ├── videos/   ← 에피소드 .mp4
#       └── logs/     ← EVAL-libero_object_temp-{DATE_TIME}--normal_libero_pro_object.txt
#
# 실행:
#   bash /path/to/VLA_Adapter/script/eval_libero_pro_normal.sh
...
RESULT_ROOT="/path/to/VLA_Adapter/eval/libero_pro_env_object/Normal_libero_pro_object"
GPU="${GPU:-3}"
...
/path/to/VLA_Adapter/vla_adapter_env/bin/python run_libero_pro_eval.py \
    ...
    --seed                   7 \
    --run_id_note            normal_libero_pro_object
